...
$parentModule = 'ProgramManager'
if (-not $ModuleName) { $ModuleName = 'ProgramManager.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\ProgramManager'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\ProgramManager.psd1"
$prereqHash = @{
...
if ($LocalRepo)
{
	...
	Write-PSFMessage -Level Important -Message "Creating Nuget Package for module: ProgramManager"
	New-PSMDModuleNugetPackage -ModulePath $workingRoot.FullName -PackagePath . -EnableException
}
else
{
	# Publish to Gallery
	Write-PSFMessage -Level Important -Message "Publishing the ProgramManager module to $($Repository)"
